 ...
 * @package    Controller
 */
class StrippedForm extends Hybrid
{
	 ...
	 * @var string
	 */
	protected $strTemplate = 'stripped_form';
	...
	public function __construct($objDatabase, $formId){
	...
	public function getFormReviewInterface($blnReview=false)
	{
		if ($blnReview)
			return;

		$objTemplate = new IsotopeTemplate('iso_form_order_review');
		$objTemplate->headline = $this->subject;

		if (is_array($_SESSION['FORM_DATA']))
		{
			foreach( $_SESSION['FORM_DATA'] as $name => $value )
			{
				$arrFormData[] = array('name'=>$name,'value'=>$value);
			}
		}
		...
		if (is_array($_SESSION['FILES']))
		{
			...
			{
				$arrFormData[] = array('name'=>$name,'value'=>$this->Environment->base . str_replace(TL_ROOT . '/', '', dirname($file['tmp_name'])) . '/' . rawurlencode($file['name']));
			}
		}
			
		
		$objTemplate->formdata = $arrFormData;			
	...
	protected function compile()
	{
		...
		$formId = strlen($this->formID) ? $this->formID : 'auto_form_'.$this->id;
		...
		while ($objFields->next())
		{
			...
			if ($objWidget->name != '')
			{
				if ($_SESSION['FORM_DATA'][$objWidget->name]){
					$objWidget->value = $_SESSION['FORM_DATA'][$objWidget->name];
				}
				$_SESSION['FORM_DATA'][$objWidget->name] = $objWidget->value;
			}
		...
		// Process form data
		if ($this->Input->get('status') == 'confirm' && !$this->doNotSubmit && $this->Input->post('FORM_SUBMIT')==$formId && $this->Input->post('FORM_CONFIRM') == 'iso_checkout_' . $this->id )
		//if ($this->Input->post('FORM_SUBMIT') == $formId && !$this->doNotSubmit && $this->Input->get('status') == 'confirm')
		{
		...
		$this->Template->hasError = $this->doNotSubmit;
