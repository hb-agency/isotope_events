 ...
 */
class ModuleEventCart extends ModuleIsotope
{
	 ...
	 * @var string
	 */
	protected $strTemplate = 'mod_iso_eventcart';
   ...
   /**
	...
	public function generate()
	{
		if (TL_MODE == 'BE')
		{
			...
			$objTemplate->wildcard = '### ISOTOPE ECOMMERCE: EVENT CART ###';
	...
	protected function compile()
	{
		...
		$lastAdded = ($this->iso_continueShopping && count($_SESSION['ISO_CONFIRM'])) ? $this->Isotope->EventCart->lastAdded : 0;

		foreach ($arrProducts as $i => $objProduct)
		{
			// Remove product from cart
			if ($this->Input->get('remove') == $objProduct->cart_id && $this->Isotope->EventCart->deleteProduct($objProduct))
			{
			...
			{
				$blnReload = true;
				$this->Isotope->EventCart->updateProduct($objProduct, array('product_quantity'=>$arrQuantity[$objProduct->cart_id]));
		...
		$blnInsufficientSubtotal = ($this->Isotope->Config->cartMinSubtotal > 0 && $this->Isotope->Config->cartMinSubtotal > $this->Isotope->EventCart->subTotal) ? true : false;
