 ...
 * Class EventCart
 * 
 * Provide methods to handle Event cart.
 * @author     Russell Winans <russ@winanscreative.com>
 */
class EventCart extends IsotopeProductCollection
{
	 ...
	 * @var string
	 */
	protected $strTable = 'tl_event_cart';

	/**
	 * Name of the child table
	 * @var string
	 */
	protected $ctable = 'tl_event_cart_items';

	/**
	 * Name of the temporary cart cookie
	 * @var string
	 */
	protected $strCookie = 'EVENT_TEMP_CART';
	...
	public function initializeCart($intConfig, $intStore)
	{
		...
		if (!FE_USER_LOGGED_IN || !$this->User->id)
		{
			...
			$objCart = $this->Database->execute("SELECT * FROM tl_event_cart WHERE session='{$this->strHash}' AND store_id=$intStore");
		}
		else
		{
			$objCart = $this->Database->execute("SELECT * FROM tl_event_cart WHERE pid={$this->User->id} AND store_id=$intStore");
		}
	...
	public function getShippingSurcharge($arrSurcharges)
	{
		...
		if ($this->Isotope->EventCart->hasShipping && $this->Isotope->EventCart->Shipping->price != 0)
		{
			$strSurcharge = $this->Isotope->EventCart->Shipping->surcharge;

			$arrSurcharges[] = array
			(
				'label'			=> ($GLOBALS['TL_LANG']['MSC']['shippingLabel'] . ' (' . $this->Isotope->EventCart->Shipping->label . ')'),
				'price'			=> ($strSurcharge == '' ? '&nbsp;' : $strSurcharge),
				'total_price'	=> $this->Isotope->EventCart->Shipping->price,
				'tax_class'		=> $this->Isotope->EventCart->Shipping->tax_class,
				'before_tax'	=> ($this->Isotope->EventCart->Shipping->tax_class ? true : false),
			);
		}
	...
	public function getPaymentSurcharge($arrSurcharges)
	{
		...
		if ($this->Isotope->EventCart->hasPayment && $this->Isotope->EventCart->Payment->price != 0)
		{
			$strSurcharge = $this->Isotope->EventCart->Payment->surcharge;

			$arrSurcharges[] = array
			(
				'label'			=> ($GLOBALS['TL_LANG']['MSC']['paymentLabel'] . ' (' . $this->Isotope->EventCart->Payment->label . ')'),
				'price'			=> ($strSurcharge == '' ? '&nbsp;' : $strSurcharge),
				'total_price'	=> $this->Isotope->EventCart->Payment->price,
				'tax_class'		=> $this->Isotope->EventCart->Payment->tax_class,
				'before_tax'	=> ($this->Isotope->EventCart->Payment->tax_class ? true : false),
			);
		}
	...
	public function getSurcharges()
	{
		...
		if (isset($GLOBALS['EVENT_HOOKS']['checkoutSurcharge']) && is_array($GLOBALS['EVENT_HOOKS']['checkoutSurcharge']))
		{
			foreach ($GLOBALS['EVENT_HOOKS']['checkoutSurcharge'] as $callback)
			{
				if ($callback[0] == 'EventCart')
				{
